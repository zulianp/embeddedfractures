...
for cond in conds:
    ...
    for title, ref in zip(titles, refinement_index):
        ...
        method = "reference"
        label = "reference"
        data = f"./results/{case}/USTUTT/MPFA/dol_cond_" + str(cond) + "_refinement_4.csv"
        plot.plot_over_line(data, label, ref, title, cond, ax,
                            plot.linestyle[place][method], plot.color[place][method],
                            has_legend=False, fmt=fmt)

    # save figures
    plot.save(cond, f"{case}_pol_cond_"+str(cond), starting_from=3*cond)
...
for cond in conds:
    ...
    plot.save(cond, f"{case}_pol_cond_"+str(cond)+"_legend")
    plot.crop_pdf(f"{case}_pol_cond_"+str(cond)+"_legend")
